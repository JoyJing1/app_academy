class ComputerPlayer
  ...
  def outcome(start_pos, end_pos)
    ...
    color = (board[start_pos].color == :white ? :black : :white)

    capture = board[end_pos].color == color
    dupped_board.move!(start_pos, end_pos)

    return :checkmate if dupped_board.checkmate?(color)
    return :check if dupped_board.in_check?(color)
